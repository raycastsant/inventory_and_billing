...
class OrdenventasController extends Controller
{
    ...
    public function actionView($id) {
        ...
        if (Yii::$app->request->post()) {
            $msgpart = "";
            if(isset(Yii::$app->request->post()['OrdenVenta']['vehiculo_id'])) {
                $model->vehiculo_id = Yii::$app->request->post()['OrdenVenta']['vehiculo_id'];
                $msgpart = "vehículo";
            }
    ...
    public function actionUpdate($id) {
        ...
        if($model->ordenVenta->estadoOrden->estado == ESTADOS::ABIERTO) {
            ...
            $areas = ArrayHelper::map(Area::find()->where(['eliminado'=>false])->asArray()->all(), 'id', 'nombre');
            $vehiculos = [''=>''];
            if(isset($model->ordenVenta->vehiculo->cliente->id))
                $vehiculos = VehiculosController::getVehiculosCliente($model->ordenVenta->vehiculo->cliente->id);     //$vehiculos = ArrayHelper::map(Vehiculo::find()->having(['cliente_id'=> $model->ordenVenta->vehiculo->cliente->id])->asArray()->all(), 'id', 'chapa', 'modelo');

            return $this->render('update', ['model' => $model, 'clientes' => $clientes, 'vehiculos'=>$vehiculos, 'areas' => $areas]);
        }
        else {
    ...
    private function getOfertasAjaxQuery() {
        ...
        /*$query->select('orden_ventas.id, orden_ventas.codigo, orden_ventas.fecha_iniciada, clientes.nombre as cliente');
        $query->innerJoin('estado_orden', 'orden_ventas.estado_orden_id=estado_orden.id')
