...
class OrdenserviciosController extends Controller //_OrdenController
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['create', 'update', 'delete', 'index', 'view', 'imprimir-oferta', 
    ...
    public function actionView($id) {
        ...
        if (Yii::$app->request->post() && isset(Yii::$app->request->post()['OrdenServicio']['vehiculo_id'])) {
            $model->vehiculo_id = Yii::$app->request->post()['OrdenServicio']['vehiculo_id'];
            if($model->save()) {
                TrazasServiciosController::insert("Actualizar vehículo de la orden, código: ".$model->codigo, $model->id);
    ...
    public function actionCreate() {
       ...
       // $dataProvider = $searchModel->search(Yii::$app->request->queryParams);->productosOrdenServicios
        ...
        if ( ($m_estado = EstadoOrden::findOne(['estado'=>ESTADOS::ABIERTO]) ) !== null) {
             return $this->render('create', ['model' => $model, 'clientes' => $clientes, 
    ...
    public function actionUpdate($id) {
        ...
        if($model->ordenServicio->estadoOrden->estado == ESTADOS::ABIERTO) {
            ...
            if(isset($model->ordenServicio->vehiculo->cliente->id))
                $vehiculos = VehiculosController::getVehiculosCliente($model->ordenServicio->vehiculo->cliente->id);    //$vehiculos = ArrayHelper::map(Vehiculo::find()->having(['cliente_id'=> $model->ordenServicio->vehiculo->cliente->id])->asArray()->all(), 'id', 'chapa', 'modelo');
            else
            if(isset(Yii::$app->request->post()['cliente']))   //Este caso es cuando falla alguna validacion para mostrar en el combo el ultimo cliente seleccionado. Recordar que en la BD el campo que se guarda es el vehiculo no el cliente
                Yii::$app->session->set('cliente', Yii::$app->request->post()['cliente']);

            $servicios = ArrayHelper::map(Servicio::find()->having(['eliminado'=> false])->orderBy('nombre')->asArray()->all(), 'id', 'nombre');
            $trabajadores = ArrayHelper::map(Trabajador::find()->having(['eliminado'=> false])->orderBy('nombre')->asArray()->all(), 'id', 'nombre');
            $areas = ArrayHelper::map(Area::find()->where(['eliminado'=>false])->asArray()->all(), 'id', 'nombre');

           // foreach($model->ordenServicio->productosOrdenServicios as $prodOrden) {
                //$prodOrden->existencia = $prodOrden->producto->existencia;
            //    $prodOrden->validate();
            //}
     
            return $this->render('update', ['model' => $model, 'clientes' => $clientes, 'servicios' => $servicios, 
                                 'trabajadores' => $trabajadores, 'vehiculos'=>$vehiculos, 'areas' => $areas]);
        }
        else {
    ...
    public function actionImprimirOferta($ordenid, $moneda2id, $printfirma='0') {
        if(isset($ordenid)) {
            ...
            $content = $this->renderPartial('_print-oferta-pdf', ['model' => $model, 'cambio' => $cambio, 
    ...
    public function actionImprimirFactura($ordenid, $moneda2id, $printfirma='0') {
        if(isset($ordenid)) {
            ...
            $ceoInfo = CeoInfosController::getCeoInfo(); // $this->findCeoInfo();
            $content = $this->renderPartial('_print-factura-pdf', ['model' => $model, 'cambio' => $cambio, 
                                                                   'moneda_salida' => $moneda_salida, 'ceoInfo' => $ceoInfo]);
    ...
    /*protected function findCeoInfo() {
        if ( ($model = CeoInfo::findOne(['id' => 2]) ) !== null) {
            return $model;
       }
    ...
    private function getOfertasAjaxQuery() {
        ...
        $query->innerJoin('vehiculos', 'orden_servicios.vehiculo_id=vehiculos.id')
              ->innerJoin('estado_orden', 'orden_servicios.estado_orden_id=estado_orden.id')
              ->innerJoin('clientes', 'vehiculos.cliente_id=clientes.id');
    ...
    private function getFacturasPendQuery() {
        $query = OrdenServicio::find();
       // $query->joinWith(['vehiculo', 'estadoOrden'])->leftJoin('clientes', 'vehiculos.cliente_id=clientes.id');
       $query->select('orden_servicios.id, orden_servicios.codigo, orden_servicios.fecha_iniciada, clientes.nombre as cliente');
       $query->innerJoin('vehiculos', 'orden_servicios.vehiculo_id=vehiculos.id')
             ->innerJoin('estado_orden', 'orden_servicios.estado_orden_id=estado_orden.id')
             ->innerJoin('clientes', 'vehiculos.cliente_id=clientes.id');
