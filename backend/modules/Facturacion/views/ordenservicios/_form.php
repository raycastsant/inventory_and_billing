...
use kartik\select2\Select2;
   ...
   //Modal para la selección del producto
    Modal::begin([
        ...
        //echo '<div id="prod_selector" class="col-md-5 hidden">';
       // echo '<legend>Seleccionar</legend>';
       /* echo    '<label>Buscar:';
...
<div class="orden-servicio-form">
        ...
        <div class="well">
            <fieldset>
                <div class="row">
                    <div class="col-md-3">
                        <?php 
                        echo $form->field($model->ordenServicio, 'codigo')->textInput(['maxlength' => true, 'readonly'=>'readonly'])->label('Código').'&nbsp;&nbsp;';  ?>
                    </div>
                    <div class="col-md-3">
                        <?php  
                        if(isset($model->ordenServicio->vehiculo->cliente->id))
                           $model->ordenServicio->cliente = $model->ordenServicio->vehiculo->cliente->id;
                       else
                       if( Yii::$app->session->get('cliente') != null )
                           $model->ordenServicio->cliente = Yii::$app->session->get('cliente');
                    ...
                    <?php
                         ...
                         $fechaini =  date("Y-m-d");
                         echo $form->field($model->ordenServicio, 'fecha_iniciada')->hiddenInput(['value' => $fechaini])->label(false);

                      /*  echo $form->field($model->ordenServicio, 'cliente')->dropDownList($clientes,
                                        ['name'=>'cliente', 
                                            'onchange'=>'
                                                $.post( "'.Yii::$app->urlManager->createUrl('facturacion/vehiculos/ajaxlist?id=').'"+$(this).val(), function( data ) {
                                                    document.getElementById("ordenservicio-vehiculo_id").innerHTML = data;
                        ...
                        echo $form->field($model->ordenServicio, 'vehiculo_id')->dropDownList($vehiculos)->label('Vehículo').'&nbsp;&nbsp;';   
                        */
                    ?>
            </fieldset>
        </div>
        ...
        <div class="row">
            ...
            <div id="productsForm" class="well">
                <fieldset>
                    ...
                    <?php
                    $productosOrden = new ProductosOrdenServicio();
                    ...
                    echo '<div class="table-responsive">';
                        ...
                        echo '<tr>';
                            echo '<th>Producto</th>';
                            echo '<th>Cantidad</th>';
                            echo '<th>Precio (cuc)</th>';
                            echo '<td>&nbsp;</td>';
                        echo '</tr>';
                        echo '</thead>';
                            echo '<tbody>';
                            
                            foreach ($model->productosOrdenServicios as $key => $_prodsOrdens) {
                                echo '<tr>';
            ...
            <!-- Form de seleccion de servicios-trabajadores -->
            <div id="serviciosForm" class="well">
                <fieldset>
                    <legend>Servicios prestados
                        <?php
                        echo Html::a('Insertar servicio', 'javascript:void(0);', [
                        'id' => 'add-servicio-btn',
                        'class' => 'pull-right btn btn-primary btn-small'
                        ])   ?>
                    </legend>
                ...
                <?php
                    $servOrden = new ServicioTrabajador();
                    $servOrden->loadDefaultValues();
                    echo '<div class="table-responsive">';
                        echo '<table id="servicios-orden" class="table table-condensed">';
                        echo '<thead>';
                        echo '<tr>';
                            echo '<th>Servicio</th>';
                        ...
                        echo '</thead>';
                            ...
                            foreach ($model->servicioTrabajadors as $key => $_servOrden) {
                                echo '<tr>';
                                echo $this->render('_form-serviciostrab-orden-servicios', [
                                'key' => $_servOrden->isNewRecord ? (strpos($key, 'new') !== false ? $key : 'new' . $key) : $_servOrden->id,
                                'form' => $form,
                                'serviciosOrden' => $_servOrden,
                                'servicios' => $servicios,
                            ...
                            echo '<tr id="new-servicio-orden-block" style="display: none;">';
                            echo $this->render('_form-serviciostrab-orden-servicios', [
                            'key' => '__id__',
                            'form' => $form,
                            'serviciosOrden' => $servOrden,
                            'servicios' => $servicios,
                            'trabajadores' => $trabajadores,
                            ]);
    ...
    <?php ob_start(); // output buffer the javascript to register later ?>
       ...
       //-------------------------- Scripts de Productos ----------------------------//     
                ...
                if (!Yii::$app->request->isPost && $model->ordenServicio->isNewRecord)
               ...
               //Método de llenado de los datos de los productos
                ...
                $(document).on('click', '.prod_input', function () {
                    if($('#modal_table').hasClass('in') == true) {
                        $input = this;
                        // $('#prod_selector').removeClass('hidden');
                            //$('#productsForm').addClass('col-md-7');
                            
                        // $('#modal_table').style="display: block; padding-right: 17px;";
                        // $('#modal_table').addClass('in');
                        $('#modal_launch').click();
                    }
                });
                ...
                //document.getElementById("modal_table").children[0].children[0].children[1].style.maxHeight = '200px';
               // console.log(document.getElementById("modal_table").children[0].children[0].children[1]);

               /* $(document).on('click', '.data_table_btn', function () {
                ...
                $('#tproductos tbody').on('click', 'tr', function () {
                   ...
                   //document.getElementById('existencia_'+$input.id).value = table.rows('.selected').data()[0][3];  //Campo existencia para las validaciones
                   
                   $(this).toggleClass('selected');
                   $('#close_modal').click();
                });

        //-------------------------- Scripts de Servicios-Trabajadores ----------------------------//
           // add servicio button
           var servOrden_k = <?php echo isset($key) ? str_replace('new', '', $key) : 0; ?>;
                $('#add-servicio-btn').on('click', function () {
                    servOrden_k += 1;
                    $('#servicios-orden').find('tbody')
                    .append('<tr>' + $('#new-servicio-orden-block').html().replace(/__id__/g, 'new' + servOrden_k) + '</tr>');
                });

                // remove servicio button
                $(document).on('click', '.remove-servOrden-btn', function () {
                    $(this).closest('tbody tr').remove();
                });
