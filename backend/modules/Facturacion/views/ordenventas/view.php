    ...
    <div class="modal-dialog modal-md" style="min-width:300px">
        <div class="modal-content animated bounceInTop">
            ...
            <div class="modal-header">
                ...
                <h4 class="modal-title text-left">Actualizar vehículo</h4>
            </div>
            <div class="modal-body well">       
                <?= $form->field($model, 'vehiculo_id')->widget(Select2::classname(), [
                    'data' => Array(),
                    'language' => 'es',
                    'options' => ['placeholder' => '-Seleccionar Vehículo-', 'id' => 'vehiculo_id',],
                    'pluginOptions' => [
                        'allowClear' => false
                    ], ])->label('Vehículo'); ?>
            </div>
            ...
            <div class="form-group">
                <button type="button" id="okBtnVehiculo" class="btn btn-success">Aceptar</button>
            </div>
            <?php ActiveForm::end(); ?>
        </div>
    </div>
...
<div class="orden-servicio-view">
    ...
    <div class="tabbable"> 
        ...
        <div class="tab-content">
                        ...
                        [
                            ...
                            'value'=> function($model) {
                                if(isset($model->vehiculo)) 
                                    return '<b>Modelo:</b>  '.$model->vehiculo->modelo.', <b>Marca:</b>  '.$model->vehiculo->marca.', <b>Chapa:</b>  '.$model->vehiculo->chapa.'     '.
            ...
            /**Lista de productos */ 
                echo $this->render('_lista-productos',
                                        [
                                            'model' => $model, 
                                            'cambio' => $cambio, 
    ...
    <script>
        ...
        $(document).on('click', '.btn_edit_vehiculo', function () {  
            $.getJSON("<?= Yii::$app->urlManager->createUrl('facturacion/vehiculos/ajaxlist?id='.$model->cliente->id) ?>", function (jsondata) {
                document.getElementById("vehiculo_id").innerHTML = jsondata;
            });

          /*  $.post("<?php // echo Yii::$app->urlManager->createUrl('facturacion/vehiculos/ajaxlist?id='.$model->cliente->id); ?> ", function( data ) {
                                        document.getElementById("vehiculo_id").innerHTML = data; });   */

            $('#updateVehicleDialog').modal('show');
        });

        $('#okBtnVehiculo').on('click', function () {
            ...
            if(vehiculo!=null && vehiculo.trim().length>0) {
                ...
                if(vehiculo==null || vehiculo.trim().length <= 0) {
                    $('#ordenventa-vehiculo_id').closest('.field-ordenventa-vehiculo_id')[0].classList.add("has-error");
                    $('#ordenventa-vehiculo_id').closest('.field-ordenventa-vehiculo_id')[0].children[3].innerHTML = "Seleccione el vehículo";
                }
            }
        });
