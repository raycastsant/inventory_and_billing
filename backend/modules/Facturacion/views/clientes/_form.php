...
<div class="cliente-form">
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="tab1">
                ...
                <div id="ClientesForm" class="well">
                    <div class="row">
                        <div class="col-md-4">
                            <?= $form->field($model->cliente, 'tipo_cliente_id')->widget(Select2::classname(), [
                                'data' =>  $tipoclientes,
                                'language' => 'es',
                                'pluginOptions' => [
                                    'allowClear' => false
                                ], ])->label('Categoría');  //$form->field($model->cliente, 'tipo_cliente_id')->dropDownList($tipoclientes)->label('Tipo de cliente');  //<?= $form->field($model, 'tipo_cliente_id')->textInput() ?>
                        ...
                        <div class="col-md-4">
                            ...
                            <?php //echo $form->field($model->cliente, 'codigo')->textInput(['maxlength' => true])->label('Código') ?>
                    </div>    
                </div>
            ...
            <div class="tab-pane" id="tab2">
                <!-- Form de seleccion de los vehiculos -->
                <div id="vehiculosForm" class="well">
                    <fieldset>
                        <legend>
                            <?php
                            echo Html::a('Insertar', 'javascript:void(0);', [
                            'id' => 'add-vehiculo-btn',
                        ...
                        <?php
                            ...
                            $vehiculo->loadDefaultValues();
                            echo '<div class="table-responsive">';
                                echo '<table id="vehiculos" class="table table-condensed table-bordered">';
                                echo '<thead>';
                                echo '<tr>';
                                    echo '<th>Chapa</th>';
                                    echo '<th>Modelo</th>';
                                    ...
                                    echo '<th>Año</th>';
                                    echo '<th>Código-motor</th>';
                                    echo '<th>Código-alternador</th>';
                                    echo '<td>&nbsp;</td>';
                                echo '</tr>';
                                echo '</thead>';
                                    echo '<tbody>';
                                    
                                    foreach ($model->vehiculos as $key => $_vehiculo) {
                                        echo '<tr>';
                                        echo $this->render('_form-vehiculos-cliente', [
                                        'key' => $_vehiculo->isNewRecord ? (strpos($key, 'new') !== false ? $key : 'new' . $key) : $_vehiculo->id,
                                        'form' => $form,
                                        'vehiculo' => $_vehiculo,
                                        'annos' => $annos,
                                        ]);
                                    ...
                                    echo '<tr id="new-vehiculo-block" style="display: none;">';
                                    echo $this->render('_form-vehiculos-cliente', [
                                    'key' => '__id__',
                                    'form' => $form,
                                    'vehiculo' => $vehiculo,
    ...
    //-------------------------- Scripts de Vehiculos ----------------------------//
            // add vehiculo button
            var vehiculo_k = <?php echo isset($key) ? str_replace('new', '', $key) : 0; ?>;
            $('#add-vehiculo-btn').on('click', function () {
                vehiculo_k += 1;
                $('#vehiculos').find('tbody')
                .append('<tr>' + $('#new-vehiculo-block').html().replace(/__id__/g, 'new' + vehiculo_k) + '</tr>');
            });

            // remove vehiculo button
            $(document).on('click', '.remove-vehiculo-btn', function () {
                $(this).closest('tbody tr').remove();
            });

            <?php
                //Mostrar campos para nuevo Vehiculo
                if (!Yii::$app->request->isPost && $model->cliente->isNewRecord)
                    echo "$('#add-vehiculo-btn').click();";
            ?>
